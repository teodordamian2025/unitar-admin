...
VALUES (
  ...
  'Factura {{numar_factura}} pentru clientul {{client_nume}} ({{client_cui}}) nu a putut fi trimisă la iapp.ro după {{retry_count}} încercări.\n\nUltima eroare: {{ultima_eroare}}\n\nVă rugăm verificați și retrimiți manual din aplicație.',
  '<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
    <div style="background: #e74c3c; color: white; padding: 15px; border-radius: 8px 8px 0 0;">
      <h2 style="margin: 0;">⚠️ Trimitere factură eșuată</h2>
    </div>
    <div style="background: #fff; border: 1px solid #ddd; padding: 20px; border-radius: 0 0 8px 8px;">
      <p>Factura <strong>{{numar_factura}}</strong> pentru clientul <strong>{{client_nume}}</strong> ({{client_cui}}) nu a putut fi trimisă la iapp.ro.</p>
      <div style="background: #fef0f0; border-left: 4px solid #e74c3c; padding: 15px; margin: 15px 0;">
        <strong>Încercări:</strong> {{retry_count}}/3<br>
        <strong>Ultima eroare:</strong> {{ultima_eroare}}
      </div>
      <p>Vă rugăm accesați aplicația pentru a retrimite manual factura.</p>
      <a href="{{link_actiune}}" style="display: inline-block; background: #3498db; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; margin-top: 10px;">Vezi factura</a>
    </div>
