// ==================================================================
// COMPONENTA: Modal Incasare Facturi Emise ANAF
// Data: 2026-01-08
// Descriere: Modal pentru marcare incasari la facturi emise (inclusiv externe)
// Similar cu IncasareModal dar adaptat pentru FacturiEmiseANAF_v2
// ==================================================================
...
interface IncasareEmiseModalProps {
  factura: FacturaEmisa;
  isOpen: boolean;
  onClose: () => void;
...
export default function IncasareEmiseModal({
  factura,
  isOpen,
  onClose,
  onSuccess
}: IncasareEmiseModalProps) {
  const [activeTab, setActiveTab] = useState<TabType>('tranzactie');
  const [loading, setLoading] = useState(false);
  const [searchLoading, setSearchLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // State pentru tab tranzactie
  ...
  const searchTranzactii = async () => {
    ...
    try {
      const params = new URLSearchParams({
        factura_emisa_id: factura.id,
        toleranta: toleranta.toString()
      });

      const response = await fetch(`/api/iapp/facturi-emise/incasare?${params}`);
      const data = await response.json();

      if (data.success) {
        setTranzactii(data.tranzactii || []);
      } else {
        setError(data.error || 'Eroare la cautarea tranzactiilor');
      }
    } catch (err) {
      console.error('Eroare cautare tranzactii:', err);
      setError('Eroare la cautarea tranzactiilor bancare');
    } finally {
    ...
    try {
      ...
      if (activeTab === 'tranzactie') {
        if (!selectedTranzactie) {
          setError('Selectati o tranzactie bancara');
          setLoading(false);
          return;
        }
        ...
        if (!valoare || valoare <= 0) {
          setError('Introduceti o valoare valida');
          setLoading(false);
          return;
        }
        if (valoare > restDePlata * 1.05) {
          setError('Valoarea depaseste restul de plata');
          setLoading(false);
          return;
        }
      ...
      const response = await fetch('/api/iapp/facturi-emise/incasare', {
        method: 'POST',
      ...
      if (!result.success) {
        throw new Error(result.error || 'Eroare la inregistrarea incasarii');
      }
      ...
      console.error('Eroare incasare:', err);
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              ...
              <p style={{ margin: '4px 0 0', color: 'rgba(255,255,255,0.85)', fontSize: '13px' }}>
                Factura {factura.serie_numar || 'N/A'}
              </p>
            </div>
          </div>
          <button
        ...
        <div style={{ padding: '20px 24px', borderBottom: '1px solid #e5e7eb' }}>
          ...
          >
            <div>
              ...
              {factura.cif_client && (
                <div style={{ fontSize: '12px', color: '#6c757d' }}>CUI: {factura.cif_client}</div>
              )}
            </div>
            <div>
              <div style={{ fontSize: '12px', color: '#6c757d', marginBottom: '4px' }}>Total factura</div>
              <div style={{ fontSize: '14px', fontWeight: '600', color: '#2c3e50' }}>
                {formatCurrency(valoareRon)}
              </div>
              {platit > 0 && (
                <div style={{ fontSize: '12px', color: '#27ae60' }}>
                  Platit: {formatCurrency(platit)}
                </div>
              )}
            </div>
            <div>
              <div style={{ fontSize: '12px', color: '#6c757d', marginBottom: '4px' }}>Rest de plata</div>
            ...
            <div>
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <label style={{ fontSize: '13px', color: '#6c757d' }}>Toleranta:</label>
                  <select
                    value={toleranta}
              ...
              {searchLoading ? (
                <div style={{
                  ...
                  <span style={{ marginLeft: '12px' }}>Se cauta tranzactii...</span>
                </div>
              ) : tranzactii.length === 0 ? (
                <div style={{
                  textAlign: 'center',
                  ...
                  <div style={{ fontSize: '14px' }}>
                    Nu s-au gasit tranzactii bancare potrivite
                  </div>
                  <div style={{ fontSize: '12px', marginTop: '8px' }}>
                    Incercati sa mariti toleranta sau folositi incasarea manuala
                  </div>
                </div>
              ) : (
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                          ...
                          <div style={{ textAlign: 'right' }}>
                            ...
                            <div style={{
                              ...
                              Diferenta: {tranzactie.diferenta_procent.toFixed(1)}%
                            </div>
                          </div>
                        </div>
                      </div>
            ...
            <div>
              <div style={{
                ...
                <div style={{ fontSize: '13px', color: '#166534' }}>
                  <strong>Incasare manuala</strong>
                  <br />
                  <span style={{ fontSize: '12px' }}>
                    Folositi aceasta optiune pentru incasari in numerar sau cand nu aveti tranzactie bancara.
                  </span>
                </div>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={2}
                  placeholder="Ex: Incasare numerar, chitanta nr. ..."
                  style={{
                    width: '100%',
                    padding: '12px 16px',
                    fontSize: '14px',
        ...
        <div style={{
          ...
          {activeTab === 'tranzactie' && selectedTranzactie && (
            <div style={{
              ...
              <div style={{ fontSize: '13px', color: '#1e40af' }}>
                <strong>Tranzactie selectata:</strong> {formatCurrency(selectedTranzactie.suma)}
          ...
          {activeTab === 'manual' && parseFloat(valoareManuala) > 0 && (
            <div style={{
              ...
              <div style={{ fontSize: '13px', color: '#166534' }}>
                <strong>Incasare manuala:</strong> {formatCurrency(parseFloat(valoareManuala))}
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            ...
            >
              {loading ? (
                ...
                <>
                  <CheckCircle size={18} />
                  {activeTab === 'tranzactie' ? 'Confirma Match' : 'Inregistreaza Incasare'}
                </>
              )}
            </button>
          </div>
