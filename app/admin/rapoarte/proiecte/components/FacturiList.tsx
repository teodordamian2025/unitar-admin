...
export default function FacturiList({ 
  ...
  // ✅ NOUĂ: Funcție safe pentru formatarea datelor
  const formatDateSafe = (dateInput: string) => {
    ...
    try {
      ...
      if (dateInput.includes('T')) {
        ...
        return new Date(dateInput).toLocaleDateString('ro-RO');
      } else if (dateInput.includes('-') && dateInput.length === 10) {
        // Format: 2025-07-25
        return new Date(dateInput + 'T00:00:00').toLocaleDateString('ro-RO');
      } else {
        // Alte formate
        return new Date(dateInput).toLocaleDateString('ro-RO');
      }
  ...
  // ✅ FIX: Download PDF care funcționează cu numele real de fișier
  const handleDownload = async (factura: Factura) => {
    try {
      // Încearcă să găsească numele real al fișierului
      const response = await fetch(`/api/actions/invoices/get-pdf-filename?facturaId=${factura.id}&numar=${factura.numar}`);
      const data = await response.json();
      
      if (data.success && data.fileName) {
        // Descarcă cu numele real
        const link = document.createElement('a');
        link.href = `/api/actions/invoices/download-pdf?fileName=${encodeURIComponent(data.fileName)}`;
